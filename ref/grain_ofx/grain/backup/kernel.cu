...
__device__
static void initCurandState(curandState* state, unsigned int x, unsigned int y, unsigned int offset) noexcept {
    const unsigned int period = 65536u; // 65536 = 2^16
    unsigned int seed = ((y % period) * period + (x % period)) + offset;
    if (seed == 0u) seed = 1u;

    // This will initialize the curand state using the computed seed
    // and some values based on the thread's unique ID to make it more random.
...
// Xorshift algorithm from George Marsaglia's paper
   unsigned int state = *p * 747796405u + 2891336453u;
   unsigned int word = ((state >> ((state >> 28u) + 4u)) ^ state) * 277803737u;
   *p = (word >> 22u) ^ word;
   return *p;
}

__device__ void mysrand(curandState *state, const unsigned int seed) {
    curand_init(seed, blockIdx.x * blockDim.x + threadIdx.x, 0, state);
}

__device__ float myrand_uniform_0_1(curandState *state) {
    return curand_uniform(state);
}

__device__ float myrand_gaussian_0_1(curandState *state) {
    return curand_normal(state);
}

__device__
static int my_rand_poisson(curandState *state, float lambda, float prod) noexcept {
    /* Inverse transform sampling using curand for uniform RNG */
...
*/
__device__
static float sqDistance(float x1, float y1, float x2, float y2) noexcept {
...
__global__ void grainKernel(int p_Width, int p_Height, float p_muR, float p_sigmaR, float p_NmonteCarlo, float p_sigmaFilter, int p_seed, float* p_Input, float* p_Output)
{
	 ...
	 if( (x<p_Width) && (y<p_Height) )
    {
		...
		curandState pMonteCarlo;
		initCurandState(&pMonteCarlo, x, y, p_seed);
		...
		for (int i=0; i<p_NmonteCarlo; i++)
		{
			...
			for(unsigned int ncx = minX; ncx <= maxX; ncx++) // x-cell number
			{
				...
				for(unsigned int ncy = minY; ncy <= maxY; ncy++) // y-cell number
				{
                    ...
                    initCurandState(&localState, ncx, ncy, p_seed);

					// Compute the Poisson parameters for the pixel that contains (x,y)
					float4 src_im_sxsy = tex2D(tex_src_im, (int)max(floor(xGaussian),0.0), (int)max(floor(yGaussian),0.0));
					u = src_im_sxsy.x;
                    int uInd = (int)floorf(u * (1024.001f) );
					...
					unsigned int Ncell = my_rand_poisson(&localState, currLambda, currExpLambda);

					for(unsigned int k=0; k<Ncell; k++)
					{
						//draw the grain centre
						//changed to float precision to avoid incorrect operations
						double xCentreGrain = (float)cellCornerX + ((float)ag)*((float)myrand_uniform_0_1(&localState));
				        double yCentreGrain = (float)cellCornerY + ((float)ag)*((float)myrand_uniform_0_1(&localState));

						//draw the grain radius
						if (p_sigmaR>0.0)
						{
							//draw a random Gaussian radius, and convert it to log-normal
							currRadius = (float)fmin((float)exp(mu + sigma*myrand_gaussian_0_1(&localState)),maxRadius);
...
__host__ void getTextureMap(float *p_Input, float* p_Output, int p_Width, int p_Height, float p_muR, float p_sigmaR, float p_NmonteCarlo, float p_sigmaFilter, int p_seed)
{
	...
	float *lambdaList = new float[MAX_FLOAT_LEVELS];
	float *expLambdaList = new float[MAX_FLOAT_LEVELS];
	for (int i=0; i<=MAX_FLOAT_LEVELS; i++)
	{
		float u = ((float)i)/(1024.001f);
		float ag = 1/ceil(1/(p_muR));
...
extern "C" void RunCudaKernel(int p_Width, int p_Height, float p_muR, float p_sigmaR, float p_NmonteCarlo, float p_sigmaFilter, int p_seed, float p_blend, float* p_Input, float* p_Output)
{
    dim3 threads(32, 4);
    dim3 blocks((p_Width + threads.x - 1) / threads.x, (p_Height + threads.y -1) / threads.y);
    //cudaStream_t stream = static_cast<cudaStream_t>(p_Stream);

    getTextureMap(p_Input + 0, p_Output + 0, p_Width, p_Height, p_muR/2, p_sigmaR, p_NmonteCarlo, p_sigmaFilter, p_seed);
    getTextureMap(p_Input + 1, p_Output + 1, p_Width, p_Height, p_muR/2, p_sigmaR, p_NmonteCarlo, p_sigmaFilter, p_seed);
    getTextureMap(p_Input + 2, p_Output + 2, p_Width, p_Height, p_muR, p_sigmaR, p_NmonteCarlo, p_sigmaFilter, p_seed + 25);
